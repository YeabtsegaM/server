...
exports.FinancialReportingUtility = void 0;
 ...
 */
class FinancialReportingUtility {
    /**
     * Get financial summary for a specific cashier
     */
    static async getCashierFinancialSummary(cashierId, startDate, endDate) {
        try {
            const query = { cashierId };
            if (startDate || endDate) {
                query['gameData.completedAt'] = {};
                if (startDate)
                    query['gameData.completedAt'].$gte = startDate;
                if (endDate)
                    query['gameData.completedAt'].$lte = endDate;
            }
            const completedGames = await CompletedGame_1.default.find(query).sort({ 'gameData.completedAt': -1 });
            const summary = {
     ...
     */
    static async getDailyFinancialSummary(date) {
        try {
            const startOfDay = new Date(date);
            startOfDay.setHours(0, 0, 0, 0);
            const endOfDay = new Date(date);
            endOfDay.setHours(23, 59, 59, 999);
            const completedGames = await CompletedGame_1.default.find({
     ...
     */
    static async getTopGamesByStake(limit = 10) {
        try {
            const topGames = await CompletedGame_1.default.find({})
     ...
     */
    static async getFinancialPerformanceOverTime(startDate, endDate, groupBy = 'day') {
        try {
            const completedGames = await CompletedGame_1.default.find({
...
exports.FinancialReportingUtility = FinancialReportingUtility;
exports.default = FinancialReportingUtility;
//# sourceMappingURL=financialReporting.js.map