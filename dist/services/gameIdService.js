...
class GameIdService {
     ...
     */
    static async getCurrentGameId(cashierId) {
        try {
     ...
     * @throws Error if cashier not found or database error
     */
    static async getNextGameId(cashierId) {
        try {
     ...
     */
    static async resetGameId(cashierId) {
        try {
            const cashier = await this.getCashierWithValidation(cashierId);
     ...
     */
    static async synchronizeWithDatabase(cashierId) {
        try {
     ...
     */
    static async getGameIdInfo(cashierId) {
        try {
            const cashier = await this.getCashierWithValidation(cashierId);
            const today = this.getStartOfDay(new Date());
     ...
     * @throws Error if cashier not found or database error
     */
    static async incrementGameIdForNextGame(cashierId) {
        try {
            // CRITICAL FIX: First synchronize with database to ensure accuracy
     ...
     */
    static async getNextGameInfo(cashierId) {
        try {
            const cashier = await this.getCashierWithValidation(cashierId);
            const today = this.getStartOfDay(new Date());
            const isNewDay = this.shouldResetGameId(cashier, today);
     ...
     */
    static validateAndNormalizeGameId(gameId) {
     ...
     */
    static async findNextAvailableGameId(cashierId, today) {
        try {
            ...
            while (attempts < maxAttempts) {
                ...
                const existingGame = await Game_1.default.findOne({
                    cashierId,
     ...
     */
    static async countGamesToday(cashierId, today) {
        try {
            const count = await Game_1.default.countDocuments({
                cashierId,
                createdAt: { $gte: today }
            });
            return count;
     ...
     */
    static async getCashierWithValidation(cashierId) {
        const cashier = await Cashier_1.default.findById(cashierId);
        if (!cashier) {
     ...
     */
    static updateCache(cashierId, gameId) {
...
exports.GameIdService = GameIdService;
// Cache for current game IDs to reduce database calls
GameIdService.gameIdCache = new Map();
GameIdService.CACHE_TTL = 5 * 60 * 1000; // 5 minutes
GameIdService.MIN_GAME_ID = 4000; // Start from 4000
GameIdService.MAX_GAME_ID = 4999; // Maximum 4999 (4-digit)
//# sourceMappingURL=gameIdService.js.map