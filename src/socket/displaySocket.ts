...
export class DisplaySocketHandler {
  private gameService: GameService;
  private io: any;
  private displaySockets: Map<string, Socket> = new Map();
  ...
  async handleConnection(socket: Socket, token: string) {
    ...
    try {
      ...
      const cashierId = (cashier._id as any).toString();
  ...
  private setupEventHandlers(socket: Socket, token: string, cashierId: string) {
    ...
    socket.on('join_display_room', (data: { sessionId: string }) => {
      if (data.sessionId) {
    ...
    socket.on('leave_game', (data: LeaveGameData) => {
    ...
    socket.on('cartela:verified', (data: any) => {
    ...
    socket.on('get_game_data', async (data: EmptyData) => {
      if (token) {
        try {
          ...
          const existingGame = await Game.findOne({ sessionId: token });
          if (!existingGame) {
    ...
    socket.on('get_cartela_selections', async (data: EmptyData) => {
      if (token) {
        try {
          const Game = (await import('../models/Game')).default;
    ...
    socket.on('cashier:connection_status', async (data: { connected: boolean; sessionId: string }) => {
      if (token) {
        try {
    ...
    socket.on('display:status_update', (data: DisplayStatusData) => {
      if (token) {
    ...
    socket.on('game_status_changed', (data: { status: string; gameData?: any }) => {
     
      // The display will handle this event to update its game state
    });
    
    // Handle game ended event
    socket.on('game_ended', (data: { gameId: string; status: string; timestamp: Date }) => {
     
      // The display will handle this event to update its game state
    });

         // Handle comprehensive game reset event
    socket.on('game_comprehensive_reset', (data: { newGameId: string; message: string; timestamp: Date }) => {
     
      // The display will handle this event to reset for new game
     });
     
          // Handle page refresh request
    socket.on('refresh_pages', (data: { message: string; timestamp: Date }) => {
     
      // The display will handle this event to refresh the page
    });

    // Handle close cartelas event
    socket.on('close_cartelas', (data: { timestamp: Date }) => {
     
      // The display will handle this event to close cartela display
    });

    // Handle placed bets updated event
    socket.on('placed_bets_updated', (data: { placedBetCartelas: number[]; gameId: string; timestamp: Date }) => {
   ...
   */
  private setupFastUpdateHandlers(socket: Socket, token: string): void {
    // Handle immediate game data updates
    socket.on('request_immediate_update', async (data: { sessionId: string }) => {
      if (data.sessionId) {
        try {
    ...
    socket.on('request_page_refresh', async (data: { sessionId: string; reason?: string }) => {
      if (data.sessionId) {
        try {
          const { RealTimeUpdateService } = await import('../services/realTimeUpdateService');
          const realTimeService = new RealTimeUpdateService(this.io);
    ...
    socket.on('shuffle_number_pool', async (data: { sessionId: string; cashierId: string }) => {
      if (data.sessionId) {
        try {
    ...
    socket.on('display:shuffle_animation', async (data: { sessionId: string; cashierId: string; action: string }) => {
      if (data.sessionId) {
        try {
   ...
   */
  private setupHeartbeat(socket: Socket, token: string, cashierId: string): void {
