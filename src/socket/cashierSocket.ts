...
interface EmptyData {
  // Empty interface for events that don't need data
}

interface DisplayStatusData {
  // Empty interface for display status events
}

export class CashierSocketHandler {
  private gameService: GameService;
  private io: any;
  private selectedCartelas: Map<string, Set<number>> = new Map();
  private lastEmittedSelections: Map<string, Set<number>> = new Map(); // Track last emitted selections
  
  // Performance optimization: Cache game data to reduce database queries
  private gameCache: Map<string, { data: any; timestamp: number }> = new Map();
  private readonly CACHE_TTL = 1000; // 1 second cache TTL
  
  // Performance optimization: Batch database operations
  private pendingDbUpdates: Map<string, { updates: any[]; timer: NodeJS.Timeout | null }> = new Map();
  ...
  private lastSocketCheckTime: Map<string, number> = new Map();
  ...
  private async getGameData(sessionId: string, forceRefresh: boolean = false): Promise<any> {
    const now = Date.now();
  ...
  private scheduleDbUpdate(sessionId: string, update: any): void {
    ...
    pending.timer = setTimeout(async () => {
      try {
        ...
        const combinedUpdate: any = { lastActivity: new Date() };
  ...
  async handleConnection(socket: Socket, cashierId: string, sessionId: string) {
    if (!cashierId) {
      ...
      if (sessionId) {
        ...
        try {
          ...
          if (game && game.gameData && game.gameData.selectedCartelas) {
            ...
            const selectedCartelas = game.gameData.selectedCartelas as number[];
   ...
   */
  private calculateTotalWinStack(gameData: any): number {
  ...
  private setupEventHandlers(socket: Socket, cashierId: string, sessionId: string) {
    // Room management
    socket.on('join_cashier_room', (data: JoinRoomData) => {
      socket.join(`cashier:${data.cashierId}`);
    });
    
    socket.on('leave_cashier_room', (data: LeaveRoomData) => {
      socket.leave(`cashier:${data.cashierId}`);
    });

    // Verification modal control
    socket.on('close-verification-modal', (data: { sessionId: string }) => {
    ...
    // Game events
    socket.on('draw_number', async (data: DrawNumberData) => {
    ...
    socket.on('auto_draw_initialize', async (data: { config?: any }) => {
    ...
    socket.on('auto_draw_config_update', async (data: { config: any }) => {
    ...
    socket.on('pause_game', async (data: GameData) => {
      if (sessionId) {
    ...
    socket.on('resume_game', async (data: GameData) => {
      if (sessionId) {
        await this.gameService.updateGameStatus(sessionId, 'active', data.gameData);
        this.gameCache.delete(sessionId);
      }
    ...
    socket.on('end_game', async (data: GameData) => {
      if (sessionId) {
        ...
        let cashierIdForNewGame: string | null = null;
        try {
          const currentGame = await this.getGameData(sessionId);
          if (currentGame) {
    ...
    socket.on('reset_game', async (data: GameData) => {
      if (sessionId) {
    ...
    socket.on('get_game_data', async (data: EmptyData) => {
      if (sessionId) {
    ...
    socket.on('get_display_status', async (data: DisplayStatusData) => {
      if (sessionId) {
        const game = await this.getGameData(sessionId);
        if (game) {
    ...
    socket.on('sync_now', async (data: EmptyData) => {
      if (sessionId) {
        try {
    ...
    socket.on('show_cartelas_on_display', async (data: { sessionId: string; cartelas: number[] }) => {
      if (sessionId) {
    ...
    socket.on('get_cartela_selections', async (data: { sessionId: string }) => {
      if (sessionId) {
        try {
    ...
    socket.on('select_cartela', async (data: { sessionId: string; cartelaId: number }) => {
    ...
    socket.on('deselect_cartela', async (data: { sessionId: string; cartelaId: number }) => {
      ...
      if (sessionId) {
        try {
          ...
          try {
            ...
            if (currentGame && currentGame.gameData) {
              ...
              if (totalWinStack !== currentGame.gameData.totalWinStack) {
                await Game.findOneAndUpdate(
                  { sessionId },
                  { 'gameData.totalWinStack': totalWinStack },
                  { new: true }
                );
          ...
          if (updatedGame.gameData) {
           ...
           try {
             const GameAggregationService = (await import('../services/gameAggregationService')).default;
    ...
    socket.on('clear_all_cartelas', async (data: { sessionId: string }) => {
      if (sessionId) {
    ...
    socket.on('close_cartela_display', async (data: { sessionId: string }) => {
      if (sessionId) {
    ...
    // Bet events
    socket.on('create_bet', async (data: any) => {
      if (sessionId) {
    ...
    socket.on('get_display_status', async (data: { sessionId: string }) => {
      if (data.sessionId) {
        try {
          const game = await this.getGameData(data.sessionId);
          const displayConnected = game?.connectionStatus?.displayConnected || false;
    ...
    socket.on('display:connected', async (data: { token: string }) => {
    ...
    socket.on('display:connection_status', async (data: { connected: boolean; sessionId: string }) => {
    ...
    socket.on('game:status_update', async (data: { sessionId: string; status?: string; gameId?: string }) => {
      if (data.sessionId === sessionId) {
    ...
    socket.on('game_status_changed', async (data: { sessionId: string; newStatus: string; gameData?: any }) => {
      if (data.sessionId === sessionId) {
  ...
  private setupConnectionStatusCheck(socket: Socket, cashierId: string, sessionId: string) {
      ...
      try {
        ...
        if (game && game.connectionStatus) {
          ...
          if (now - lastSocketCheck > 10000) { // Check every 10 seconds
            ...
            if (!this.lastSocketCheckTime) this.lastSocketCheckTime = new Map();
  ...
  private getConsistentGameId(game: any, fallbackGameId?: string): string {
  ...
  private setupRealTimeSync(socket: Socket, sessionId: string): void {
  ...
  private async joinGameRoom(socket: Socket, sessionId: string, cashierId: string) {
  ...
  private async handleDisconnect(cashierId: string, sessionId: string) {
    try {
      const CashierModel = (await import('../models/Cashier')).default;
  ...
  public async clearSelectedCartelas(sessionId: string): Promise<void> {
    if (this.selectedCartelas.has(sessionId)) {
      this.selectedCartelas.delete(sessionId);
  ...
  private haveSelectionsChanged(sessionId: string, newSelections: Set<number>): boolean {
  ...
  private updateLastEmittedSelections(sessionId: string, selections: Set<number>): void {
