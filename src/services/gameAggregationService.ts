...
export class GameAggregationService {
   ...
   */
  static async aggregateGameData(sessionId: string, gameId: string) {
    try {
      
      // Get current game to check timestamp
      const currentGame = await Game.findOne({ sessionId, gameId });
      if (!currentGame) {
   ...
   */
  static async updateGameWithAggregatedData(sessionId: string, gameId: string) {
    try {
      ...
      const aggregatedData = await this.aggregateGameData(sessionId, gameId);
         ...
         { sessionId, gameId }, // Also filter by gameId for extra safety
         {
           $set: {
   ...
   */
  static async getRealTimeGameData(sessionId: string, gameId: string) {
    try {
      // CRITICAL FIX: Always require gameId to prevent old bet data contamination
      if (!gameId) {
        throw new Error('Game ID is required to prevent data contamination between games');
   ...
   */
  static async moveGameToCompleted(sessionId: string, gameId: string) {
    try {
