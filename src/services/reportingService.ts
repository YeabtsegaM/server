...
export class ReportingService {
   ...
   */
  static async getGamePerformanceSummary(startDate: Date, endDate: Date) {
    try {
      const games = await CompletedGame.find({
      ...
      games.forEach(game => {
        ...
        // Group by cashier
        const cashierName = (game.cashierId as any)?.fullName || 'Unknown';
   ...
   */
  static async getCashierPerformanceReport(cashierId: string, startDate: Date, endDate: Date) {
    try {
      const games = await CompletedGame.find({
        cashierId,
        'gameData.completedAt': {
   ...
   */
  static async getGameStatistics(gameId: string | number) {
    try {
      const game = await CompletedGame.findOne({ gameId }).populate('cashierId', 'fullName username');
      ...
      return {
        gameId: game.gameId,
        cashier: (game.cashierId as any)?.fullName || 'Unknown',
   ...
   */
  static async getRevenueTrends(startDate: Date, endDate: Date, groupBy: 'day' | 'week' | 'month' = 'day') {
    try {
      const games = await CompletedGame.find({
        'gameData.completedAt': {
          $gte: startDate,
   ...
   */
  static async getTopPerformingCashiers(limit: number = 10, startDate?: Date, endDate?: Date) {
    try {
      ...
      const cashierStats = await CompletedGame.aggregate([
        { $match: matchStage },
        {
          $group: {
            _id: '$cashierId',
      ...
      const cashiers = await Cashier.find({ _id: { $in: cashierIds } }, 'fullName username');

      return cashierStats.map(stat => {
        const cashier = cashiers.find((c: any) => c._id.toString() === stat._id.toString());
        return {
          cashierId: stat._id,
