...
import { GameIdService } from './gameIdService';
import { generateGameId, formatGameIdForDisplay } from '../utils/gameIdUtils';
...
interface GameSession {
  ...
  gameData?: {
    ...
    winPatterns?: any[];
    verifiedCartelas?: number[]; // Add verified cartelas tracking
    verificationResults?: Record<string, any>; // Add verification results tracking
    drawHistory?: any[];
    gameStartTime?: Date;
...
class GameService {
  ...
  private autoDrawController: AutoDrawController;

  constructor(io: Server) {
    this.io = io;
    this.autoDrawController = new AutoDrawController(this);
    console.log('🎮 GameService initialized with AutoDrawController');
  }

  // Create or get existing game session
  async createGameSession(cashierId: string, sessionId: string, displayToken: string): Promise<GameSession> {
    ...
    const gameId = generateGameId(nextGameId);
  ...
  async joinGameRoom(socket: any, sessionId: string, displayToken: string, cashierId?: string): Promise<GameSession | null> {
    const roomName = `game:${sessionId}`;
    ...
    if (!gameSession) {
      ...
      const existingGameInDb = await Game.findOne({ sessionId });
      ...
      if (cashierId) {
        ...
        if (gameSession.gameId === "4999" || gameSession.gameId === "0") {
          ...
          const nextGameId = await GameIdService.getNextGameId(cashierId);
          const newGameId = generateGameId(nextGameId);
          
          // Delete the waiting game and create a new one
          await Game.deleteOne({ sessionId });
  ...
  async createWaitingGameSession(sessionId: string, displayToken: string): Promise<GameSession> {
    const roomName = `game:${sessionId}`;
  ...
  public async broadcastGameDataUpdate(sessionId: string): Promise<void> {
    try {
  ...
  // Update game status
  async updateGameStatus(sessionId: string, status: string, gameData?: any): Promise<void> {
    try {
      const gameSession = this.activeGames.get(sessionId);
  ...
  async startGame(sessionId: string, selectedCartelas: number[]): Promise<void> {
    try {
      const gameSession = this.activeGames.get(sessionId);
      if (!gameSession) {
  ...
  async recordNumberDraw(sessionId: string, number: number, drawnBy: 'manual' | 'auto' = 'manual'): Promise<void> {
    const gameSession = this.activeGames.get(sessionId);
    if (!gameSession) {
    ...
    if (drawnBy === 'auto') {
      ...
      if (cashierId) {
        ...
        const drawnNumbers = numberPoolService.getDrawnNumbers(cashierId);
   ...
   */
  async initializeAutoDraw(cashierId: string, sessionId: string, config?: any): Promise<void> {
   ...
   * Sync number pool with current game state
   */
  private async syncNumberPoolWithGame(cashierId: string, sessionId: string): Promise<void> {
    try {
      const Game = (await import('../models/Game')).default;
      const game = await Game.findOne({ sessionId });
      ...
      if (game?.gameData?.calledNumbers) {
        const calledNumbers = game.gameData.calledNumbers;
        numberPoolService.syncPoolWithGameState(cashierId, calledNumbers);
   ...
   */
  private async saveAutoDrawConfig(cashierId: string, sessionId: string, config: any): Promise<void> {
    try {
      const Game = (await import('../models/Game')).default;
      await Game.findOneAndUpdate(
        { sessionId },
   ...
   */
  async loadAutoDrawConfig(cashierId: string, sessionId: string): Promise<any> {
    try {
      const Game = (await import('../models/Game')).default;
      const game = await Game.findOne({ sessionId });
   ...
   */
  async updateAutoDrawConfig(cashierId: string, config: any, sessionId?: string): Promise<boolean> {
  ...
  getNumberPoolStats(cashierId: string): any {
    return numberPoolService.getCashierPoolStats(cashierId);
  }
  ...
  shuffleNumberPool(cashierId: string): void {
    numberPoolService.shuffleCashierPool(cashierId);
  ...
  async updateConnectionStatus(sessionId: string, type: 'cashier' | 'display', connected: boolean): Promise<void> {
  ...
  async emitConnectionStatusUpdate(sessionId: string): Promise<void> {
    try {
      const gameSession = this.activeGames.get(sessionId);
  ...
  async disconnectGameSession(sessionId: string): Promise<void> {
    const gameSession = this.activeGames.get(sessionId);
    if (!gameSession) return;
  ...
  async resetGame(sessionId: string): Promise<void> {
    ...
    console.log(`🔄 Game reset for session ${sessionId} - Game ID: ${formatGameIdForDisplay(gameSession.gameId)}`);
  }

  // Get active games for admin
  async getActiveGames(): Promise<any[]> {
    const activeGames = await Game.find({ isConnected: true })
    ...
    return activeGames.map(game => ({
      ...
      cashierName: (game.cashierId as any)?.fullName || 'Unknown',
  ...
  async getGameData(sessionId: string): Promise<any> {
    const game = await Game.findOne({ sessionId });
    if (!game) return null;

    return {
      id: formatGameIdForDisplay(game.gameId),
      gameId: game.gameId,
      status: game.status,
      gameData: game.gameData,
      connectionStatus: game.connectionStatus,
